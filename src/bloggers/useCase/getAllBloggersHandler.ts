...
export class GetAllBloggersHandler implements IQueryHandler<GetAllBlogsCommand> {
    ...
    async execute(query: GetAllBlogsCommand): Promise<any> {
        ...
        const blogs = await this.blogsRepo.getUsersBlogsPaginated(blogsPaginationBLLdto, userIdFromToken)
        ...
        const mappedBlogs = await this.blogsRepo
            .mapBlogsToResponse(blogs, 'id', 'name', 'youtubeUrl', 'createdAt')
        const docCount = await this.blogsRepo.countUsersBlogsBySearchname(searchNameTerm, userIdFromToken)
        const result = {
