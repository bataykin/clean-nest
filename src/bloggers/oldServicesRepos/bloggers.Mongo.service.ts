...
export class BloggersMongoService implements ABloggersService{
    constructor(
        protected readonly bloggersRepo: BloggersMongoRepo
        // protected readonly bloggersRepo: BloggersSQLRepo
    ) {
    ...
    async findAll({searchNameTerm = null, pageNumber = 1, pageSize = 10}: BlogsPaginationDto) {
        ...
        const mappedBloggers = bloggers.map((b) => {
            return {id: b.id, name: b.name, youtubeUrl: b.youtubeUrl}
        })
    ...
    async create(dto: CreateBloggerDto) {
        ...
        const mappedBlogger = newBlogger.map((b) => {
            return {id: b._id, name: b.name, youtubeUrl: b.youtubeUrl}
        })
    ...
    async findById(id: string) {
        try {
            ...
            if(e.name == 'NotFoundException') {
                ...
                throw new NotFoundException('bloggerID (CastError), give me real ObjectID of bloggerID')
            }
    ...
    async update(id: string, dto: UpdateBlogDto) {
        try {
            ...
            if(e.name == 'NotFoundException') {
                ...
                throw new NotFoundException('CastError, mf, give me real ObjectID of bloggerID')
            }
            else throw new BadRequestException(e)
        }
