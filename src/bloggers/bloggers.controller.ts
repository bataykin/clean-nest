...
export class BloggersController {
    ...
    @HttpCode(200)
    async getAllCommentsOnMyBlog(@Query() dto: BlogsPaginationDto,
                                 @Request() req) {
        const accessToken = req.headers.authorization?.split(' ')[1]
        return this.queryBus.execute(new GetAllCommentsOnMyBlogCommand(dto, accessToken))
    }
    ...
    @UseGuards(JwtAuthGuard)
    @HttpCode(204)
    async removeBlog(@Param('id', ParseUUIDPipe) id: string,
                     @Request() req) {
        const accessToken = req.headers.authorization?.split(' ')[1]
    ...
    async updateBlog(@Param('id', ParseUUIDPipe) id: string,
    ...
    @UseGuards(JwtAuthGuard)
    async createPostByBlog(@Param('blogId', ParseUUIDPipe) blogId: string,
                           @Body() dto: CreatPostByBlogDto,
                           @Request() req) {
        const accessToken = req.headers.authorization?.split(' ')[1]
        return this.commandBus.execute(new CreatePostByBlogCommand(blogId, dto, accessToken))
    }
    ...
    @UseGuards(JwtAuthGuard)
    @HttpCode(HttpStatus.NO_CONTENT)
    async updatePostByBlog(@Param('blogId', ParseUUIDPipe) blogId: string,
                           @Param('postId', ParseUUIDPipe) postId: string,
                           @Body() dto: UpdatePostByBlogDto,
                           @Request() req) {
        const accessToken = req.headers.authorization?.split(' ')[1]
        return this.commandBus.execute(new UpdatePostByBlogCommand(blogId, postId, dto, accessToken))
    }
    ...
    @UseGuards(JwtAuthGuard)
    @HttpCode(HttpStatus.NO_CONTENT)
    async deletePostByBlog(@Param('blogId', ParseUUIDPipe) blogId: string,
                           @Param('postId', ParseUUIDPipe) postId: string,
                           @Request() req) {
        const accessToken = req.headers.authorization?.split(' ')[1]
        return this.commandBus.execute(new DeletePostByBlogCommand(blogId, postId, accessToken))
    }

    @Post('blogs')
    @UseGuards(JwtAuthGuard)
    async createBlog(@Body(new ValidationPipe({
    ...
    @UseGuards(JwtAuthGuard)
    @HttpCode(200)
    async getMyBlogs(@Query() dto: BlogsPaginationDto,
                     @Request() req) {
        const accessToken = req.headers.authorization?.split(' ')[1]
    ...
    @UseGuards(JwtAuthGuard)
    @HttpCode(HttpStatus.NO_CONTENT)
    async banUnbanUserByBlog(@Param('id', ParseUUIDPipe) userId: string,
                             @Body() dto: BanUserByBlogDto,
                             @Request() req) {
        const accessToken = req.headers.authorization?.split(' ')[1]
        return this.queryBus.execute(new BanUnbanUserByBlogCommand(userId, dto, accessToken))
