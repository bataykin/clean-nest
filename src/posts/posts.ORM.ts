...
export class PostsORM extends Repository<PostEntity>
    ...
    async findPostById(id: string): Promise<PostEntity | null> {
        const result = await this.postsRepo.findOneBy({id, blogger: {isBanned: false}})
        return result
    }

    async getPostByBloggerId(bloggerId: string, userId: string, dto: PaginationPostsDto): Promise<PostEntity | null> {
        return Promise.resolve(undefined);
    }

    async getPostsPaginated(dto: PaginationPostsDto): Promise<PostEntity[]> {
        let posts = await this.postsRepo
            .createQueryBuilder('posts')
            .leftJoinAndSelect('posts.blogger', 'blogs')
            .where("blogs.isBanned = false")
            .skip(dto.skipSize)
            .take(dto.pageSize)
            .orderBy('posts.' + dto.sortBy,
