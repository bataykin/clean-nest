...
export class UsersORM extends Repository<UserEntity>
    ...
    async confirmEmail(code: string) {
        const confirm = await this.usersRepo.update({confirmationCode: code}, {isConfirmed: true})
    ...
    async createUser(login: string, email: string, passwordHash: string, code: string): Promise<UserEntity> {
    ...
    async updateConfirmationCode(email: string, code: string) {
        const confirm = await this.usersRepo.update({email: email}, {confirmationCode: code})
    ...
    async checkPassRecoveryCodeIsValid(recoveryCode: string) {
        const isExists = await this.usersRepo.findOneBy({passwordRecoveryCode: recoveryCode})
        if (!isExists) {
            return null
        } else {
            return isExists
    ...
    async countUsersBySearchname(searchNameTerm: string, searchEmailTerm: string) {
        const res = await this.usersRepo.createQueryBuilder('u')
    ...
    async setBanStatus(userId: string, dto: BanUnbanUserDto) {
        if (dto.isBanned) {
            const res = await this.usersRepo.update({id: userId},
                {isBanned: dto.isBanned, banReason: dto.banReason, banDate: new Date()})
            return res
        } else {
            const res = await this.usersRepo.update({id: userId},
