...
import {IPostsRepo} from "../posts/IPostsRepo";
...
export class CommentsORM extends Repository<CommentEntity>
    ...
    async getAllCommentByBlog(userId: string, dto: BlogsPaginationDto): Promise<CommentEntity[]> {
        console.log(userId)
    ...
    async mapCommentsToResponse(allComments: CommentEntity[]): Promise<CommentViewDto[]> {
        const result = []
        let res: CommentViewDto
        for await (let comment of allComments) {
            let {id, content, createdAt, userId, userLogin, postId} = comment
            let likes = await this.manager.createQueryBuilder()
                .select()
                .from(LikeEntity, 'l')
