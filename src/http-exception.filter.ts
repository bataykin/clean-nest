...
export class HttpExceptionFilter implements ExceptionFilter {
    constructor() {
        ...
        const message = (Array.isArray(exeptionResponse.message)) ? exeptionResponse.message[0] : exeptionResponse.message
        const field = (Array.isArray(exeptionResponse.message) && status !== 429) ? exeptionResponse.message[0].split(' ')[0] : 'field'
        const errorsMessages = []
        // console.log( exeptionResponse.message[0])
        let fieldName = ''
        if (Array.isArray(exeptionResponse.message || status !== 429)) {
            for (let err of exeptionResponse.message) {
                if (fieldName !== err[0]) {
                    fieldName = err[0]
                    errorsMessages.push({
                            message: err,
