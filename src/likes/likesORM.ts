...
export class LikesORM extends Repository<LikeEntity>
    ...
    async addReactionToComment(userId: string, commentId: string, likeStatus: LikeStatusEnum) {
        const result = await this.likesRepo.manager.upsert(LikeEntity,
            {
                userId: userId,
                commentId: commentId,
                reaction: likeStatus,
    ...
    async getUserLikeStatusPost(userId: string, postId: string) {
        const result = this.likesRepo.createQueryBuilder()
            .select('reaction')
    ...
    async getCommentLikeDislikeCounts(commentId: string) {
        const result = await this.likesRepo.createQueryBuilder('l')
    ...
    async mapLikesToCommentEntityToResponse(comment: CommentEntity, userId?: string) {
        const {postId, user, likes, ...rest} = comment
        const likesDislikesCount = await this.getCommentLikeDislikeCounts(comment.id)
        const status = (userId)
    ...
    async mapArrayCommentEntitiesToResponse(comments: CommentEntity[], userId: string) {
        const result = []
        for await (let comment of comments) {
            result.push(await this.mapLikesToCommentEntityToResponse(comment, userId))
        }
        return result
    }
