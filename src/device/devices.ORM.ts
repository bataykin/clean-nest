...
export class DevicesORM extends Repository<DeviceEntity>
    ...
    async deleteAllDevices(userId: string, deviceId: string) {
       return await this.devicesRepo.createQueryBuilder('devices')
            .delete()
            .from(DeviceEntity)
    ...
    async getAllDevices(userId: string) {
        ...
        const devs = await  this.devicesRepo.findBy({userId: userId, loggedOut: false})
        // if (!devs) return null
        // console.log(devs)
        return devs
   ...
   async  getDeviceByTitleAndIp(userId: string, devName: string, ip: string): Promise<DeviceEntity | null> {
        return await this.devicesRepo.findOneBy({userId: userId, title: devName, ip: ip} )
    }
