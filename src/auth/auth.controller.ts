...
export class AuthController {
    ...
    async renewPassword(@Body() dto: RenewPasswordDto) {
        return this.commandBus.execute(new RenewPasswordCommand(dto.newPassword, dto.recoveryCode))
    }
    ...
    async confirmRegistration(@Body('code'
        , new ParseUUIDPipe({
        errorHttpStatusCode: HttpStatus.BAD_REQUEST,
        exceptionFactory: error => {
            throw new BadRequestException({message: 'code ' + error, field: "code"})
        }
    })
    ) code: string) {
    ...
    async resendRegistrationEmail(@Body( ) email: passRecoveryDto) {
        return this.commandBus.execute(new ResendRegistrationEmailCommand(email.email))
    ...
    async login(@Request() req, @Response({passthrough: true,}) res,
        ...
        if (req.user.isBanned) {
            throw new UnauthorizedException(`user ${req.user.login} is banned, reason: ${req.user.banReason}`)
        }
        const tokens = await this.commandBus.execute(new LoginCommand(req.user, req.get('user-agent'), req.get('host')))
    ...
    async refreshTokens(@Request() req, @Response({passthrough: true}) res) {
        ...
        const tokens = await this.commandBus.execute(new RefreshTokensCommand(refToken, req.get('user-agent'), req.get('host')))
        res.cookie('refreshToken', tokens.refreshToken, {
            httpOnly: jwtConstants.httpOnly,
            secure: jwtConstants.cookieSecure,
        })
