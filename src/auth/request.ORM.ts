...
export class RequestORM extends Repository<RequestEntity>
    ...
    async addRequest(ip: string, trueDate: Date, fullUrl: string) {
        return await this.requestsRepo.createQueryBuilder()
            .insert()
            .into(RequestEntity)
    ...
    async getAttempts(ip: string, attemptsInterval: number, trueDate: Date, fullUrl: string) {
        const initTime = addSeconds(trueDate, -attemptsInterval)
        return await this.requestsRepo.createQueryBuilder('request')
            .select("COUNT(request)", "count")
            .where("request.ip = :ip", {ip: ip})
            .andWhere("request.endPoint = :url", {url: fullUrl})
            .andWhere("request.reqDate > :initTime",
                {initTime: initTime})
            .getCount()
