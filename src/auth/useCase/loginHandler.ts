...
import {BadRequestException, Body, HttpException, Inject, Response, UnauthorizedException} from "@nestjs/common";
...
export class LoginHandler implements ICommandHandler<LoginCommand> {
    ...
    async execute(command: LoginCommand): Promise<any> {
        ...
        const deviceExisted = await this.devicesRepo.getDeviceByTitleAndIp(user.id, devName, ip)
        if (!deviceExisted) {
            payload = {
                ...prePayLoad,
                deviceId: uuidv4(),
